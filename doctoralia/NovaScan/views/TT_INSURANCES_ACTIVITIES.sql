...
/****** Object:  View [dbo].[TT_INSURANCES_ACTIVITIES]    Script Date: 11/05/2022 15:18:19 ******/
SET ANSI_NULLS ON
GO
...
CONVERT(VARCHAR(50), CONV.CONVENIOID) + '_' + CONVERT(VARCHAR(50),CONV.CONVENIOCOD) + + '_' + CONVERT(VARCHAR(50),ISNULL(PLA.PLANOID,CONV.CONVENIOID)) AS INSURANCE_LID,
	CAST(PRO.PROCID AS VARCHAR) AS ACTIVITY_LID
	,0 AS ACTIVITY_PRICE -- STR(CAST(ISNULL(VLR.QUANTCH, 0)AS int) + CAST(ISNULL(VLR.QUANTCO, 0)AS int) + CAST(ISNULL(VLR.QUANTFILME, 0)AS int), 12,2) AS ACTIVITY_PRICE -- inserir preço da tabela particular <> 22 = 0
...
and PLA.SUSPENSO='F'
AND CONV.SUSPENSO='F'
AND CONV.DESCRICAO NOT LIKE '%EXCLUIDO%'
and PLA.PLANODESCRICAO NOT LIKE '%EXCLUIDO%'
