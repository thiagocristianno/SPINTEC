...
/****** Object:  Trigger [dbo].[TT_RESERVATIONS_TMP_INSERT]    Script Date: 11/05/2022 08:19:03 ******/
SET ANSI_NULLS ON
GO
...
(
		...
		,SUBSTRING(@user_landline_phone,1,2) --<DDD, varchar(5),>
...
INSERT INTO [dbo].[PACIENTETELEFONES]
           ([CONTADOR]
           ,[UNIDADEID]
           ,[PACIENTEID]
           ,[DDD]
           ...
           ,[TIPOTELEFONEID])
     VALUES
           ((select MAX(contador)+1 FROM PACIENTETELEFONES WITH(NOLOCK)) --<CONTADOR, int,>
           ,0 --<UNIDADEID, int,>
           ,@user_lid_int --<PACIENTEID, int,>
           ,SUBSTRING(@user_landline_phone,1,2) --<DDD, varchar(5),>
           ,@user_landline_phone --<TELEFONE, varchar(20),>
           ,'RESIDENCIAL' --<TIPO, varchar(11),>
           ,'5' --<TIPOTELEFONEID, int,>
		   );
     ...
     VALUES
           ...
           ,@user_mobile_phone --<TELEFONE, varchar(20),>
           ,'CELULAR' --<TIPO, varchar(11),>
           ,'1' --<TIPOTELEFONEID, int,>
		   );
...
SET @STATUS_AGENDA = (SELECT  TOP 1 A.STATUS FROM HORARIOS A WHERE  A.HORA = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'), 7,20), 3) AND DATA =  @app_date AND A.LIVROID = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'), 7,20), 4))
...
UPDATE HORARIOS SET			
		 ...
		 ,[HORA]	=  PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'), 7,20), 3)--(SELECT TOP 1 AUX.COL2 AS LIVRO FROM ##AUX_2 AUX INNER JOIN TT_RESERVATIONS_TMP T ON AUX.APP_LID = T.APP_LID) --@APP_LID --cast(@app_start_time as INT) --<HORA int >
		 ,[CONTADOR]	= 0--<CONTADOR int >
		 ,[MEDREAID]	= CASE WHEN CHARINDEX('_', @resource_lid) > 0 THEN PARSENAME(replace(@resource_lid, '_','.'), 2)  ELSE NULL END --<MEDREAID int >
...
(SELECT TOP 1
CAST(H.LIVROID AS varchar)  + '_' +  CAST(H.HORA AS varchar) + '_' +  CAST(@HORREQID AS varchar)+ '_' + CAST(HR.UNIDADEID AS varchar)+'_' + PARSENAME(replace(@activity_lid, '_','.'), 2)
FROM HORARIOS H
...
UPDATE [dbo].[HORARIOS] SET
 [CONTADOR]	= 0--<CONTADOR int >
,[MEDREAID]	= CASE WHEN CHARINDEX('_', @resource_lid) > 0 THEN PARSENAME(replace(@resource_lid, '_','.'), 2) ELSE CAST(replace(@resource_lid, 'E','') AS int) END--<MEDREAID int >
...
WHERE 
LIVROID = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 4) AND HORA = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 3) AND DATA = @app_datE AND HORREQID = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 2)
...
delete from horariosrequisicao where horreqid=PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 2) and horaagendamento = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 3) AND UNIDADEID = @location_lid
DELETE FROM HORARIOSDADOS WHERE LIVROID = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 4) AND HORA = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 3) AND DATA = @APP_DATE 
DELETE FROM HORARIOSEXAMES WHERE LIVROID = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 4) AND HORA = PARSENAME(SUBSTRING(replace(@APP_LID, '_','.'),0,18), 3) AND DATA = @APP_DATE 
END
