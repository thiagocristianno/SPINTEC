...
/****** Object:  View [dbo].[TT_INSURANCES]    Script Date: 11/05/2022 08:22:04 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[TT_INSURANCES] AS
	SELECT 
		...
		CONVERT(VARCHAR(50), C.DESCRICAO) AS INSURANCE_NAME, --+ ' - ' + P.PLANODESCRICAO),50) AS INSURANCE_NAME,		
		--CASE WHEN DESCRICAO LIKE '%PARTICULAR%' THEN '1' ELSE '2' END INSURANCE_TYPE,
		CASE WHEN FORMAPAGAMENTO IN ('P','V') THEN '1' ELSE '2' END INSURANCE_TYPE
	FROM CONVENIOS C
		LEFT JOIN CONVENIOSPLANOS P ON
		P.CONVENIOID = C.CONVENIOID
		AND P.SUSPENSO = 'F' AND P.PLANOID=C.PLANOID
	WHERE 
		C.SUSPENSO = 'F' 
