...
/****** Object:  View [dbo].[TT_RESERVATIONS]    Script Date: 11/05/2022 15:19:00 ******/
SET ANSI_NULLS ON
GO
...
SELECT
CAST(V.LIVROID AS varchar) +  '_'  + CAST(V.HORA AS varchar) + '_' + CAST(V.HORREQID AS varchar) + '_' + CAST(U.UNIDADEID AS VARCHAR) AS  APP_LID,
'' AS APP_TIME,
...
END AS USER_SECOND_NAME,
 ...
 case when P.TIPOTELEFONEID IN ('5') THEN RTRIM(P.TELEFONE) ELSE  (SELECT TOP 1 RTRIM(FONE.TELEFONE) FROM PACIENTETELEFONES FONE WHERE FONE.TIPO IN ('Residencial','') AND FONE.PACIENTEID = P.PACIENTEID)  END USER_LANDLINE_PHONE,
 
 case when P.TIPOTELEFONEID IN ('2') THEN RTRIM(P.TELEFONE) ELSE  (SELECT TOP 1 RTRIM(FONE.TELEFONE) FROM PACIENTETELEFONES FONE WHERE FONE.TIPO IN ('Comercial','') AND FONE.PACIENTEID = P.PACIENTEID)  END USER_WORK_PHONE,
 ...
 P.ENDERECO AS USER_STREET,
 P.NUMERO AS USER_STREET_NUMBER,
 P.CIDADE AS USER_CITY,
 P.BAIRRO AS USER_PROVINCE,
 P.ESTADO AS USER_REGION,
 'BRASIL' AS USER_COUNTRY,
 case when P.TIPOTELEFONEID IN ('1') THEN RTRIM(P.TELEFONE) ELSE  (SELECT TOP 1 RTRIM(FONE.TELEFONE) FROM PACIENTETELEFONES FONE WHERE FONE.TIPO IN ('Celular','CELULAR') AND FONE.PACIENTEID = P.PACIENTEID)  END USER_MOBILE_PHONE,
 '' AS USER_LANGUAGE,
 '' AS USER_PRIVACY,
 '' AS USER_PRIVACY_PROMOTIONS,
 '' AS COMMUNICATION_PREFERENCES,
 ISNULL(CAST(V.MEDREAID AS varchar), ISNULL(CAST(V.LIVROID  AS VARCHAR), '')+'E') AS RESOURCE_LID, 
 PR.PROCID AS ACTIVITY_LID,
 CONVERT(VARCHAR(50), C.CONVENIOID) + '_' + CONVERT(VARCHAR(50),C.CONVENIOCOD)  + '_'  + isnull(CONVERT(VARCHAR(50),PL.PLANOID),CONVERT(VARCHAR(50), C.CONVENIOID)) AS INSURANCE_LID,
...
END  AS STATUS,
 ...
 '' AS APP_OWNER_TID,
 '' AS APP_OWNER_NAME,
 ISNULL(P.PESO,'') AS APP_CUSTOM_0,
 ISNULL(P.ALTURA,'') AS APP_CUSTOM_1,
 ISNULL(P.CONFIGURAVEL1,'')  AS APP_CUSTOM_2,
 '' AS APP_CUSTOM_3,
 '' AS APP_CUSTOM_4
FROM VW_AGENDA V
--LEFT JOIN HORARIOS H ON H.LIVROID = V.LIVROID
FULL JOIN PACIENTE P ON P.PACIENTEID = V.PACIENTEID
LEFT JOIN PROCEDIMENTOS PR ON PR.MNEMONICO = V.MNEMONICOS
FULL JOIN CONVENIOS C ON V.CONVENIOID = C.CONVENIOID 
...
WHERE  V.DATA > '2021-04-05 00:00:00.000' --AND V.LIVROID ='8' 
